...
from sklearn.datasets import load_breast_cancer # cluster dataset create
...
n_estimators=100 : 학습 tree model 수
objective='binary:logistic'  이항 분류 다항 분류 결정(Y를 보고 자동 결정)
'''

# 2. model 학습 조기종료 : early stopping rounds
eval_set = [(x_test , y_test)] # 평가셋
model_early = xgb.fit(x_train , y_train , eval_set = eval_set , eval_metric = 'error' , early_stopping_rounds = 50 , verbose = True)
'''
X, y : 훈련 셋
eval_set : 평가 셋
