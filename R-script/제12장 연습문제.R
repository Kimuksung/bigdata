...
data <- subset(smoke, !is.na(smoking2), c(education2, smoking2)) 
data

#<단계 3> 교차분할표 작성    
CrossTable(data$education2, data$smoking2 , chisq = T)
...
#<단계 5> 검정결과 해석
# p-value <0.05보다 작기 때문에 귀무가설인 나이와 흡연은 관계가 없다 라는 가설 기각
# = > 나이와 흡연은 관계가 있다.
...
# p <0.05 이기 때문에 나이와 직위가 관련이 없다라는 귀무가설은 반박 -> 관련이 있다.
...
# [단계 3] 교차분할표 작성
CrossTable(response$job2, response$response2 , chisq = TRUE) 
...
# p-value =< 0.05 -> 직업에 따라 응답 정도가 차이가 있다.