...
## 3. 세종 사전에 신규 단어 추가
# term : 추가 단어 tag : 명사 동사 분류 ncn = 명
user_dic <- data.frame(term=c("R 프로그래밍","페이스북","김진성","소셜네트워크"), tag='ncn')

# Sejong 사전에 신규 단어 추가 : KoNLP 제공 
buildDictionary(ext_dic='sejong', user_dic = user_dic)

temp = extractNoun("나는 김진성이고,대한민국 사람이다.") #명사 추출
paste(temp , collapse = " ") #단어 vector 하나의 vector로
...
myCorpusPrepro_term <- TermDocumentMatrix(myCorpusPrepro, 
                                          control=list(wordLengths=c(4,16))) #2음절~ 8음절(음절=단어의 길이)
...
# (1) 단어 이름 생성 -> 빈도수의 이름
myName <- names(wordResult)   #출연 빈도수
