...
str(convert)
str(quakes)
xyplot(lat~long | depth2, data=quakes)
...
# number : conditioning interval( is not factor) 
# overlap : 겹치는 구간(0.1~9:작을 수록  사이 간격이 적게 겹침) 
# row : 패널 행수
...
#      hwy : 고속도로 주행 마일 수 , cty : 도시 주행 마일 수
...
qplot(wt, mpg, data=mtcars, color=carb) # color 가 num 이라면 동일 색상 농도 / factor 라면 n개의 다른 색상
...
p+geom_bar(aes(fill=cut), position="fill")  # bar 추가 / 밀도 1 기준 꽉찬 막대 함수
...
register_google(key = 'AIzaSyB1molbMBqScJoiWHQJ_c0a7Ruaqeh-00k')
...
map2 <- get_stamenmap(seoul, zoom=12,  maptype='watercolor')#'toner-2011')
ggmap(map2) # maptype : terrain, watercolor
...
map <- get_stamenmap(daegu, zoom=12,  maptype='terrain')#'toner-2011')
ggmap(map)
...
df = data.frame(region,lon,lag,tot_pot)
...
geom_text(data=df,aes(x=lon+0.01, y=lat+0.08,label=region),size=3)

#지도 이미지 file save
ggsave("pop201801.png",scale=1 ,width=10.24, height=7.68)
