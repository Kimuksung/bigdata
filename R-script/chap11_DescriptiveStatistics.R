...
length(data$cost)
summary(na.omit(data$cost)) # 요약통계량 - 의미있음(mean) - 8.784
mean(data$cost) # NA
data$cost
quantile(na.omit(data$cost), 3/4)
quantile(na.omit(data$cost), 1/4)
outlier(data$cost)
...
dim(data)
#vetor 정렬 -> sort
data[sort(data$cost),] #제대로 sorting되지 않는다.

#dataframe 정렬 -> order
data[order(data$cost),] #기존 data indexing

#dataframe 정렬 -> arrange
library(dplyr)
arrange(data,cost) #새로운 indexing
...
# 최빈수 = 중위수 = 평균 : 좌우 대칭
# 최빈수 < 중위수 = 평균 : 오른쪽 기울어짐  __/-\_
# 최빈수 > 중위수 = 평균 : 왼쪽 기울어짐 _/-\__
...
# 정규성 검정
shapiro.test(cost)
# p-value = 0.002959 < 0.05 귀무가설 기각 -> 정규분포 X
