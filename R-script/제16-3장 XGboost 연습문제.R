...
xgb_laon_model <- xgboost(data = dtrain, max_depth = 2, eta = 1, nthread = 2, nrounds = 2, objective = "binary:logistic", verbose = 0)


# 4. prediction 
pred = predict(xgb_laon_model,as.matrix(test_mat))
...
# 8. 중요변수 보기(model 비교) 
import <- xgb.importance(colnames(train_mat), model = xgb_laon_model)
