...
marketing3=as.vector(marketing2)
length(unique(marketing3))
which(duplicated(marketing3))
...
temp <- file("C:/ITWILL/2_Rwork/Part-II/temp.txt", encoding="UTF-8")
...
lword2 <- sapply(lword, unique) # 중복제거2(줄 단위 대상) 
length(lword2) 
class(lword) # list
lword # 추출 단어 확인

temp3 = Map(extractNoun, temp2) 
temp3
str(temp3)
class(temp3)
length(temp3)
