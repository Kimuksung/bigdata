...
desc member; -- 칼럼 세부 자료형 확인 가능 eclipse에서 불가능(cmd창에서만 가능)
...
insert into depart(dno,dname,daddress) values (1,'기획실','서울시');
insert into depart values (2,'영업부','싱가폴'); 
insert into depart (dno,dname) values(3,'사업부');

-- 2) select : 레코드 검색 
-- 형식) select 칼럼명 from 테이블명 [where 조건식];

select * from depart;
select * from depart where dno=1;
select * from depart where daddress is not null;

-- 3) update : 레코드 수정 
-- 형식) update 테이블명 set 칼럼명 = 값 where 조건식;

update depart set daddress='중국' where dno=3;
select * from depart;

-- 4) delete : 레코드 삭제 
-- 형식) delete from 테이블명 where 조건식;

delete from depart where dno=3;
select * from depart;
...
--ex) grant connect, create view to scott;
--ex) revoke connect, create view to scott;  
