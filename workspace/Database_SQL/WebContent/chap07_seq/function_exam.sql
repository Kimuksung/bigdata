...
-- 힌트 : substr() 함수 이용
select * from student;
select name,jumin from student where substr(JUMIN,3,2)='08';
...
select name,pay,bonus,pay*12+nvl2(bonus,bonus,0) as "연봉" from professor 
--Q4. Professor 테이블에서 교수명, 학과명을 출력하되 
--  deptno가 101번이면 ‘컴퓨터 공학과’, 102번이면 
-- ‘멀티미디어 공학과’, 103번이면 ‘소프트웨어 공학과’, 
-- 나머지는 ‘기타학과’로 출력하기
-- decode()함수 이용
select name,deptno,decode(deptno,101,'컴퓨터 공학과',102,'멀티미디어 공학과',103,'소프트웨어 공학과','기타학과') as "학과" from professor;
